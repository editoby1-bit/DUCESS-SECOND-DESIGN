(() => {
  ...
  const MODULES = {
    ...
    tellering: {
      title: 'Tellering',
      desc: 'Credit and debit customers through a journal workflow with approved form control.',
      icon: '💳',
  ...
  function renderApprovals() {
    ...
    const codRows=(state.cod||[]).filter(c=>c.status==='flagged').map((c,i)=>`<tr><td>${i+1}</td><td>${fmtDate(c.date)}</td><td>${c.staffName}</td><td>${money(c.formAmount ?? c.openingBalance ?? 0)}</td><td>${money(c.totalCredits||0)}</td><td>${money(c.totalDebits||0)}</td><td class="${Number(c.remainingBalance||c.runningFloat||0)<0?'balance-negative':''}">${money(c.remainingBalance ?? c.runningFloat ?? 0)}</td><td class="${Number(c.variance||0)<0?'balance-negative':''}">${money(c.variance||0)}</td><td class="${Number(c.overdraw||0)>0?'balance-negative':''}">${money(c.overdraw||0)}</td><td>${c.resolutionNote || c.note || '—'}</td><td>${(canCloseBusinessDay())?`<button data-cod-resolve="${c.id}" class="warning">Resolve</button>`:'Awaiting Resolution'}</td></tr>`).join('');
  ...
  function openJournalApprovalModal(reqId){
    ...
    openModal('Journal Approval', `<div class="stack"><div class="kpi-row"><div class="kpi"><div class="label">Posted By</div><div class="number">${escapeHtml(req.requestedByName)}</div></div><div class="kpi"><div class="label">Form</div><div class="number">${money(opening)}</div></div><div class="kpi"><div class="label">Total</div><div class="number">${money(total)}</div></div><div class="kpi"><div class="label">Overdraw</div><div class="number ${running<0?'balance-negative':''}">${money(Math.max(0,-running))}</div></div></div>${fieldNoteBlock}<div class="table-wrap"><table class="table"><thead><tr><th>S/N</th><th>Customer</th><th>Account</th><th>Amount</th><th>Remaining Balance</th></tr></thead><tbody>${bodyRows}</tbody></table></div></div>`, actions);
  }
  ...
  function renderStaffDirectory() {
    return `
      <div class="table-card">
        ...
        <div class="table-wrap"><table class="table"><thead><tr><th>S/N</th><th>Staff</th><th>Office</th><th>Status</th><th>Teller Account</th><th>Wallet</th><th>Debt</th><th>Remaining Balance</th><th>Action</th></tr></thead><tbody>${state.staff.map((s,i)=>{
  ...
  function openFloatTopUpModal(staffId=null) {
    ...
    `, [
      ...
      { label: 'Submit', onClick: () => {
          ...
          if (!(amount > 0)) return showToast('Enter valid form top-up');
  ...
  function bindJournal(kind) {
    ...
    if (byId('txJournalAdd')) byId('txJournalAdd').onclick = () => {
      ...
      renderWorkspace();
      setTimeout(() => {
        const pane = byId('journalPane') || document.querySelector('#journalPaneWrap');
  ...
  function bindApprovals() {
    ...
    const openCentralCodFromApprovals = byId('openCentralCodFromApprovals');
    if (openCentralCodFromApprovals) openCentralCodFromApprovals.onclick = () => confirmAction(`Close business date ${businessDate()}? This will open ${nextDate(businessDate())}.`, openCODModal);
  ...
  function openFloatModal() {
    ...
    `, [
      ...
      { label: 'Submit', onClick: () => {
          ...
          closeModal();
          render();
          showToast('Form declaration sent for approval');
      }}
    ]);
  }
  ...
  function approvalModeForRequestRow(row) {
    return String(row?.paymentMode || row?.payoutSource || 'cash').toLowerCase() === 'transfer' ? 'transfer' : 'cash';
  }

  function approvedTransactionTotalForDate(staffId, dateStr, direction, mode=null) {
    const types = direction === 'credit' ? ['customer_credit','customer_credit_journal'] : ['customer_debit','customer_debit_journal'];
    return (state.approvals||[])
      .filter(r => types.includes(r.type) && r.status === 'approved' && r.payload?.staffId === staffId && r.payload?.date === dateStr)
      .reduce((sum, r) => {
        if (r.type.endsWith('_journal')) {
          return sum + (r.payload.rows||[])
            .filter(row => !mode || approvalModeForRequestRow(row) === mode)
            .reduce((a, row) => a + Number(row.amount || 0), 0);
        }
        if (mode && approvalModeForRequestRow(r.payload) !== mode) return sum;
        return sum + Number(r.payload?.amount || 0);
      }, 0);
  }

  function approvedCreditTotalForDate(staffId, dateStr) {
    return approvedTransactionTotalForDate(staffId, dateStr, 'credit');
  }

  function approvedDebitTotalForDate(staffId, dateStr) {
    return approvedTransactionTotalForDate(staffId, dateStr, 'debit');
  }

  function approvedCreditCashTotalForDate(staffId, dateStr) {
    return approvedTransactionTotalForDate(staffId, dateStr, 'credit', 'cash');
  }

  function approvedCreditTransferTotalForDate(staffId, dateStr) {
    return approvedTransactionTotalForDate(staffId, dateStr, 'credit', 'transfer');
  }

  function approvedDebitCashTotalForDate(staffId, dateStr) {
    return approvedTransactionTotalForDate(staffId, dateStr, 'debit', 'cash');
  }

  function approvedDebitTransferTotalForDate(staffId, dateStr) {
    return approvedTransactionTotalForDate(staffId, dateStr, 'debit', 'transfer');
  }
  ...
  function openMyCODModal(selectedDate=null) {
    const st = currentStaff();
    state.ui.myCodDate ||= selectedDate || businessDate();
    const c = staffCODRecords((st||{}).id).find(x => x.date === state.ui.myCodDate);
    const summary = c ? `<div class="stack">
      <div class="kpi-row">
        <div class="kpi"><div class="label">Form</div><div class="number">${money(c.formAmount ?? c.openingBalance ?? openingBalanceOnlyForDate(c.staffId, c.date))}</div></div>
        <div class="kpi"><div class="label">Credit Cash</div><div class="number">${money(c.totalCreditsCash ?? approvedCreditCashTotalForDate(c.staffId, c.date))}</div></div>
        <div class="kpi"><div class="label">Credit Transfer</div><div class="number">${money(c.totalCreditsTransfer ?? approvedCreditTransferTotalForDate(c.staffId, c.date))}</div></div>
        ...
        <div class="kpi"><div class="label">Debit Cash</div><div class="number">${money(c.totalDebitsCash ?? approvedDebitCashTotalForDate(c.staffId, c.date))}</div></div>
        <div class="kpi"><div class="label">Debit Transfer</div><div class="number">${money(c.totalDebitsTransfer ?? approvedDebitTransferTotalForDate(c.staffId, c.date))}</div></div>
        ...
        <div class="kpi"><div class="label">Net Book Balance</div><div class="number ${Number(c.netBookBalance||0)<0?'balance-negative':''}">${money(c.netBookBalance ?? ((c.totalCredits ?? approvedCreditTotalForDate(c.staffId, c.date)) - (c.totalDebits ?? approvedDebitTotalForDate(c.staffId, c.date))))}</div></div>
        <div class="kpi"><div class="label">Remaining Balance</div><div class="number ${Number(c.remainingBalance ?? c.runningFloat ?? 0)<0?'balance-negative':''}">${money(c.remainingBalance ?? c.runningFloat ?? 0)}</div></div>
      ...
      <div class="note">Form is the approved money available for field postings. Remaining Balance = Form - Total Credits - Total Debits.</div>
    ...
    openModal('My Close of Day', `<div class="stack"><div class="action-inline"><div class="inline-field compact"><span>Close of Day Date</span><input type="date" id="myCodDate" value="${state.ui.myCodDate}"></div></div>${summary}</div>`, [{label:'Close', className:'secondary', onClick: closeModal}]);
  ...
  function openCODResolutionModal(codId) {
    ...
    const currentRemainingBalance = Number(cod.remainingBalance ?? cod.runningFloat ?? (opening - totalCredits - totalDebits));
    ...
    openModal('Resolve Close of Day', `
      <div class="stack">
        <div class="note">Form is the approved money available for field postings. Remaining Balance = Form - Total Credits - Total Debits. Final Agreed Amount corrects the system total only.</div>
        <div class="kpi-row">
          <div class="kpi"><div class="label">Form</div><div class="number">${money(cod.formAmount ?? opening)}</div></div>
          <div class="kpi"><div class="label">Credit Cash</div><div class="number">${money(cod.totalCreditsCash ?? approvedCreditCashTotalForDate(cod.staffId, cod.date))}</div></div>
          <div class="kpi"><div class="label">Credit Transfer</div><div class="number">${money(cod.totalCreditsTransfer ?? approvedCreditTransferTotalForDate(cod.staffId, cod.date))}</div></div>
          <div class="kpi"><div class="label">Total Credits</div><div class="number">${money(totalCredits)}</div></div>
          <div class="kpi"><div class="label">Debit Cash</div><div class="number">${money(cod.totalDebitsCash ?? approvedDebitCashTotalForDate(cod.staffId, cod.date))}</div></div>
          <div class="kpi"><div class="label">Debit Transfer</div><div class="number">${money(cod.totalDebitsTransfer ?? approvedDebitTransferTotalForDate(cod.staffId, cod.date))}</div></div>
